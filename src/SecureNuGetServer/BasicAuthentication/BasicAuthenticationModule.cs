...
using SecureNuGetServer.BasicAuthentication.Configuration;

namespace SecureNuGetServer.BasicAuthentication
{
  /// <summary>
  ...
  public class BasicAuthenticationModule : IHttpModule
  {
    ...
    /// <summary>
    ///   Dictionary that caches whether basic authentication challenge should be sent. Key is request URL + request method,
    ...
    /// <summary>
    ///   Exclude configuration - request URL is matched to dictionary key and request method is matched to the value of the
    ///   same key-value pair.
    /// </summary>
    ...
    public void Init(HttpApplication context)
    {
      ...
      {
        Debug.WriteLine(
          "BasicAuthenticationModule not started - Configuration not found. Make sure that BasicAuthenticationConfigurationSection section is defined.");
        return;
      }
      ...
      if (appSetting != null) isEnabled = appSetting == "true";
    ...
    public void IssueAuthenticationChallenge(object source, EventArgs e)
    {
      ...
      if (allowLocal && context.Request.IsLocal) return;

      if (allowRedirects && IsRedirect(context.Response.StatusCode)) return;
    ...
    private bool ShouldChallenge(HttpContext context)
    {
      ...
      if (shouldChallengeCache.ContainsKey(key)) return shouldChallengeCache[key];

      // if value is not found in cache check exclude rules
      foreach (var urlVerbRegex in excludes)
        if ((urlVerbRegex.Key.IsMatch(context.Request.Path) || urlVerbRegex.Key.IsMatch(context.Request.Url.Host)) &&
            urlVerbRegex.Value.IsMatch(context.Request.HttpMethod))
        {
    ...
    protected virtual bool ValidateCredentials(string userName, string password)
    {
      ...
      if (activeUsers.ContainsKey(lowerCaseUserName) && activeUsers[lowerCaseUserName] == password) return true;

      return false;
    }

    protected virtual bool ExtractBasicCredentials(string authorizationHeader, ref string username,
      ref string password)
    {
      if (string.IsNullOrEmpty(authorizationHeader)) return false;

      var verifiedAuthorizationHeader = authorizationHeader.Trim();
      if (verifiedAuthorizationHeader.IndexOf(HttpBasicSchemeName, StringComparison.InvariantCultureIgnoreCase) !=
          0) return false;

      // get the credential payload
      verifiedAuthorizationHeader = verifiedAuthorizationHeader.Substring(HttpBasicSchemeName.Length,
      ...
      var decodedAuthorizationHeader =
        Encoding.UTF8.GetString(credentialBase64DecodedArray, 0, credentialBase64DecodedArray.Length);
      ...
      password = decodedAuthorizationHeader
        .Substring(separatorPosition + 1, decodedAuthorizationHeader.Length - separatorPosition - 1).Trim();
    ...
    private void InitExcludes(BasicAuthenticationConfigurationSection basicAuth)
    {
      ...
      for (var i = 0; i < basicAuth.Excludes.Count; i++)
      {
        ...
        if (string.IsNullOrEmpty(excludeUrl)) excludeUrl = allowAnyRegex;
        if (string.IsNullOrEmpty(excludeVerb)) excludeVerb = allowAnyRegex;
